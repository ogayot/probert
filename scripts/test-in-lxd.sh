...
then
    ...
    lxc config device add $TESTER code disk source=`pwd` path=/probert
else
    lxc start $TESTER
fi
# copy is allowed to fail, in case the probert directory being tested
# includes some uncopyable stuff
lxc exec $TESTER -- sh -ec "
    cd ~
    sudo cp -a /probert . || true
    [ -d ~/probert ]
    "
